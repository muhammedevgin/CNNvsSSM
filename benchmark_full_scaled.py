...
def benchmark_ssm_original():
    print("\n" + "=" * 64)
    print("  PHASE 2: Original S-SSM (d_model=64, 1 block, 0.06M)")
    print("=" * 64)
    sys.stdout.flush()
...
def benchmark_ssm_scaled():
    ...
    print(f"  PHASE 3: SCALED S-SSM (d_model={SCALED_D_MODEL}, "
          f"{SCALED_NUM_BLOCKS} blocks, ~2.28M)")
...
def print_comparison_table(cnn, ssm_orig, ssm_scaled):
    ...
    print("\n")
    print("╔" + "═" * 78 + "╗")
    print("║" + "  COMPARISON: CNN vs Original S-SSM vs Scaled S-SSM (Param-Matched)".center(78) + "║")
    print("╠" + "═" * 78 + "╣")

    hdr = (f"║ {'Metric':<24} │ {'CNN (MobileNetV2)':>17} │ "
           f"{'S-SSM (Original)':>16} │ {'S-SSM (Scaled)':>13} ║")
    print(hdr)
    print("╠" + "═" * 24 + "═╪═" + "═" * 17 + "═╪═"
          + "═" * 16 + "═╪═" + "═" * 13 + "═╣")
